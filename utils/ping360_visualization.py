...
def quick_visualize(pings_csv: Union[str, pathlib.Path], 
                   config_csv: Optional[Union[str, pathlib.Path]] = None,
                   show_all: bool = False, **kwargs) -> Ping360Visualizer:
    """
    Quick visualization function for Ping360 data.
    
    Parameters:
    -----------
    pings_csv : str or Path
        Path to pings CSV file
    config_csv : str or Path, optional
        Path to config CSV file
    show_all : bool, optional
        Show all visualization types if True, just polar radar if False
    **kwargs
    ...
    viz.configure(**kwargs)
    viz.load_data(pings_csv, config_csv)
    viz.preprocess_data()
    
    # Always show the polar radar view (most important)
    viz.plot_polar_radar()
    
    if show_all:
        viz.plot_cartesian_projection()
        viz.plot_range_angle_improved()
        viz.plot_raw_stack()
    
    viz.verify_angle_consistency()
