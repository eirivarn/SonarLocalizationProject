...
DVL_data = pd.read_csv("../output/mc_lab_DVL/trial_1/output.csv")
...
dvl_time = DVL_data['%time'].values
dvl_distance = DVL_data['field.NetDistance'].values * 100  # Convert from meters to centimeters
dvl_pitch = np.rad2deg(DVL_data['field.NetPitch'].values)
dvl_heading = np.rad2deg(DVL_data['field.NetHeading'].values)

#For DVL time, add a comma/dot 10 digits from left
dvl_time = dvl_time / 1e9
...
bag_start_time = 1747996880.242816
...
# Adjust DVL times relative to the minimum timestamp
time_shifted_dvl = dvl_time - bag_start_time
filtered_dvl_distance = np.convolve(dvl_distance, np.ones(5)/5, mode='same')
plt.plot(time_shifted_dvl, filtered_dvl_distance, label='DVL', color='blue')
# plt.plot(time_shifted_dvl, dvl_distance, label
...
# DVL Pitch and Heading
time_shifted_dvl = dvl_time - bag_start_time
filtered_dvl_pitch = np.convolve(dvl_pitch, np.ones(5)/5, mode='same')
filtered_dvl_heading = np.convolve(dvl_heading, np.ones(5)/5, mode='same')
ax1.plot(time_shifted_dvl, filtered_dvl_pitch, label='DVL Pitch', color='blue')
# ax1.plot(time_shifted_dvl, dvl_pitch, label='DVL Raw Pitch', color='cyan')
ax2.plot(time_shifted_dvl, filtered_dvl_heading, label='DVL Heading', color='blue')
# ax2.plot(time_shifted_dvl, dvl_heading, label='DVL Raw Heading', color='cyan')
