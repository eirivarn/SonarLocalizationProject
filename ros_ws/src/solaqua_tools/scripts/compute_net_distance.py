...
# Ensure package root is on sys.path so imports of utils.* work when installed via catkin
PACKAGE_ROOT = Path(__file__).resolve().parents[1]
if str(PACKAGE_ROOT) not in sys.path:
    sys.path.insert(0, str(PACKAGE_ROOT))

PROJECT_ROOT = PACKAGE_ROOT
ROS_WS_ROOT = Path(__file__).resolve().parents[4]  # .../ros_ws
DEFAULT_BAGS_DIR = ROS_WS_ROOT / "bags"
DEFAULT_OUTPUTS_DIR = ROS_WS_ROOT / "outputs"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional ROS topic (Float32MultiArray) to stream frames instead of reading bags/rosbags.",
    )
    return parser.parse_args()


def _collect_npz_from_topic(
    topic: str,
    frame_count: int,
    run_id: str,
    exports_dir: Path,
    ...
    tmp_npz = Path(tempfile.mkstemp(prefix=f"{run_id}_stream_", suffix=".npz", dir=exports_dir)[1])
    temp_artifacts.append(tmp_npz)
    ...
    def _cb(msg: Float32MultiArray):
        ...
        if shape and arr.size >= shape[0] * shape[1]:
            arr = arr[: shape[0] * shape[1]].reshape(shape)
        frames.append(arr)
        if len(frames) >= frame_count:
            rospy.signal_shutdown("Collected requested frames")

    rospy.init_node("compute_net_distance_stream", anonymous=True, disable_signals=True)
    rospy.Subscriber(topic, Float32MultiArray, _cb, queue_size=10)
    rospy.loginfo(f"[compute_net_distance] Streaming from {topic}, target_frames={frame_count}")
    last_log = time.time()
    try:
        while not rospy.is_shutdown() and len(frames) < frame_count:
            if time.time() - last_log > 5:
                last_log = time.time()
                rospy.loginfo(f"[compute_net_distance] Frames received: {len(frames)}/{frame_count}")
            rospy.sleep(0.1)
    except KeyboardInterrupt:
        rospy.logwarn("[compute_net_distance] Interrupted; proceeding with collected frames.")

    if not frames:
        raise RuntimeError(f"No frames received from topic {topic}")

    np.savez_compressed(tmp_npz, cones=np.stack(frames, axis=0))
    stats = {
        "frames": len(frames),
        "source": "ros_topic",
        "mode": "stream",
...
def main() -> int:
    ...
    if args.ros_topic:
        npz_path, extract_stats = _collect_npz_from_topic(
            args.ros_topic,
            args.frame_count,
            bag_id,
            exports_dir,
